...
def graphlets_cholesterol_pca(r,to_csv=False):
    ...
    for column in ['chol','chol_bind','chol_chol']:
        ...
        for g,o in [('G10','21'),('G9','17'),('G3','4')]:
            ...
            f = pd.DataFrame({g:total_by_chol},index = groupby_o.mean().index)
...
def node_PCA_windowed(r,output=False,to_csv=False):
    ...
    for g,o in [('G10',[21,20,18,19]),('G9',[17,16,15]),('G3',[4,5])]:
        ...
        for n in range(1,1349):
            d = df.loc[df['node'] == n]
            dic[n] = np.mean(d[o].loc[df['chol'] == 15]) - np.mean(d[o].loc[df['chol'] == 30])/((np.std(d[o].loc[df['chol'] == 15]) + np.std(d[o].loc[df['chol'] == 30]))/2)
            # dic[n] = np.mean(d[o])
